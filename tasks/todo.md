...
- [ ] **P1 · Refactorización de frontend bloqueada por cobertura textual.** El comportamiento del frontend está protegido en un 77% por aserciones que leen el texto de `core/app.js`. 53 de 69 comprobaciones describen garantías reales sin prueba funcional independiente. Consecuencia: cualquier refactorización de `app.js` —formato, partición o módulos— rompe la suite sin que exista forma de saber si el comportamiento cambió. Bloquea las tareas 6.4 y 6.5. La [tabla completa de 69 comprobaciones, snapshot de Prettier y hashes](../docs/reference/frontend-app-js-refactor-audit/README.md) es el insumo obligatorio para retomar el trabajo. El umbral conversacional `app.js <= 1.450 líneas` queda retirado: medía las decisiones del impresor, no la complejidad.
...
- [x] **P1 · Punto ciego sistémico cuando `logs/` está inaccesible.** Resuelto: `SafeLogger` comprueba disponibilidad antes de `lifecycle.start`, comunica indisponibilidad y recuperación mediante un canal nominal independiente de auditoría, y `MainWindow` muestra un aviso no bloqueante y accesible sin rutas ni payloads. Las operaciones continúan; tras una escritura recuperada el aviso se retira y una caída posterior vuelve a avisar. `GetIntegrityReport` sigue omitiendo `reportPath` y no afirma haber persistido `integrity.report.failed` cuando el canal está caído.
...
- [x] **Deuda de pruebas · Diez aserciones textuales retiradas en 3.6, bloque 6 y 4.4.** La quinta, en `managed-area-create-workspace-harness.js`, buscaba la forma interna `ExecuteTransactionUnlocked`; tras 6.1 se retiró en vez de cambiarla por `_transactions.Execute`, porque `ManagedAreasTests.CreateWorkspaceInAreaIsAtomicAndUndoable` cubre membresía, backup, historial y undo conjunto. En 6.3 se retiraron cuatro más que buscaban mensajes, títulos y `MutateManagedAreasUnlocked`; las garantías quedan cubiertas por `CreateWorkspaceInAreaIsAtomicAndUndoable`, `CreateWorkspaceRejectsTargetAreaOnAnotherMapWithoutWrites`, `CreateWorkspaceInAreaRejectsScenarioWithoutWrites` y `DissolveReleasesMemberships`. La décima, en `detail-panel-viewport-invariance-harness.js`, exigía texto CSS que mantuviera el panel como overlay; 4.4 invierte deliberadamente ese diseño y `check-minimum-viewport.py` verifica el comportamiento mediante rectángulos en navegador real. El baseline de migración conserva 1.005 aserciones; el árbol actual tiene 997, según el recuento reproducible por fichero, tras retiradas justificadas de aserciones de forma. Regla para 6.2–6.5: ante cada arnés roto por un refactor, decidir primero si comprueba comportamiento o forma.
- [ ] **Deuda de pruebas · `managed-area-create-workspace-harness.js` es íntegramente de forma.** Auditoría 6.3b: sus 10 casos restantes / 12 aserciones sólo inspeccionan texto de `Resources/index.html`, `app.js` o `DataStore.cs`; no ejecutan DOM, puente ni almacenamiento. Mantenerlos sin tocar por no mezclar una consolidación de tests con 6.3b, pero evaluar sustituirlos por contratos funcionales sobre la infraestructura compartida de Playwright y pruebas C# del puente.\n- [ ] **Investigación pendiente · Compatibilidad de backups heredados en producción.** Las copias locales `Portable/runtime-data` y `qa-runtime-data` contienen 63 ZIP cada una, todos con `createdAtUtc`, y cero directorios heredados; no son evidencia del despliegue real. Quien tenga acceso a los datos de producción debe comprobar si existen directorios de backup heredados o manifiestos sin `createdAtUtc`. Si tampoco existen allí, planificar la retirada de esa compatibilidad para adelgazar `BackupService`; no declararla código muerto ni eliminarla antes de esa comprobación.
