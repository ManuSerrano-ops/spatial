...
- [ ] **P2 · `GetBackups()` publica manifiestos estructuralmente incompletos.** La caracterización 6.2a demuestra que un ZIP legible cuyo `manifest.json` es `{}` se lista como backup, aunque no tenga `id` ni fecha y no pueda restaurarse; en cambio, un ZIP ilegible sí se descarta. Validar la estructura mínima del manifiesto y decidir cómo informar los archivos inválidos en una tarea separada. No corregir durante la extracción de `BackupService`.
