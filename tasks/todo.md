...
- [ ] **P1 · Punto ciego sistémico cuando `logs/` está inaccesible.** Afecta a toda llamada `Info` o `Error`: `SafeLogger.Write` intenta escribir siempre en la misma carpeta y, si falla, sólo emite una vez `Console.Error` mediante su `failureIndicator`, normalmente invisible en la aplicación WPF. En `GetIntegrityReport`, la operación devuelve un informe aparentemente correcto sin `reportPath`; el frontend sólo renderiza los conteos y no muestra aviso. La caracterización 6.3a lo demuestra con `logs` como fichero: ni el informe ni `integrity.report.failed` quedan persistidos. Diseñar un canal de degradación independiente y una señal visible de operación parcial; no cambiarlo durante 6.3.
