...
Assert(selected.Length == expected.Length, $"Expected {expected.Length} exact Light logical names, found {selected.Length}: {string.Join(", ", selected.Select(item => item.Relative))}");
Assert(expected.All(path => selected.Any(item => item.Relative == path)), "Every Light logical name preserves map-themes/light.");
...
try
{
    ...
    {
        Assert(File.Exists(Path.Combine(extracted, relativePath.Replace('/', Path.DirectorySeparatorChar))), $"Extracted path is missing: {relativePath}");
    }
    Console.WriteLine($"EmbeddedResourceExtractorHarness: {expected.Length}/{expected.Length} Light paths extracted");
    return 0;
}
finally
{
