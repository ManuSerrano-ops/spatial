(() => {
  ...
  const ui = {
      state: null, adding: false, addingContext: null, moving: false, movingSeat: false, changes: [], compareUnits: [], selectedCompareUnitIds: new Set(), touchedSeats: new Set(), messageTimer: null, busyAction: null, busyTimer: null, disabledControls: new Map(), contextPoint: null, undoPayload: null, pendingSeatId: null, pendingMapId: null, pendingAreaFocusId: null, pendingClusterId: null, clusterDraft: null, cardEdit: null, cardMove: null, cardSizeUndo: null, assignmentBaseline: null, pendingWarning: null, bulkConfirmation: null,
  ...
  function clusterCardDimensions(layout, shape) { const defaults = { compact: [132, 54], square: [112, 86], vertical: [96, 104] }; const [width, height] = defaults[shape] || defaults.compact; return { width: layout.width || width, height: layout.height || height }; }
  ...
  function renderManagedAreaCard(host, map, area) {
    ...
    if (area.id === focusedAreaId) return;
      ...
      const layout = clusterCardLayout(area.id); const content = clusterCardShapeHelpers.buildClusterCardShapePresentation({ name: area.name, counts: presentation.counts, shape: layout.shape }); const dimensions = clusterCardDimensions(layout, content.shape); const members = clusterCardMemberModel(area, layout, dimensions); const anchor = clusterCardAnchor(presentation, layout); const card = document.createElement('div'); card.className = `managed-area-card cluster cluster-${content.shape}${focusedAreaId ? ' dim' : ''}${editing ? ' card-editing' : ''}${layout.cardSizingMode === 'manual' ? ' card-manual' : ''}${dimensions.width >= 420 ? ' cluster-card-wide' : ''} cluster-detail-${members.level}`; card.tabIndex = 0; card.setAttribute('role', 'button'); card.dataset.areaId = area.id; card.dataset.managedAreaId = area.id; card.dataset.mapId = area.mapId; card.style.left = `${anchor.x * 100}%`; card.style.top = `${anchor.y * 100}%`; card.setAttribute('aria-label', content.tooltip); card.title = content.tooltip; if (layout.width) card.style.width = `${layout.width}px`; if (layout.height) card.style.height = `${layout.height}px`; const directActions = editing ? '' : `<div class="cluster-card-actions" aria-label="Acciones del cluster"><button type="button" class="cluster-card-action" data-cluster-card-action="rename" aria-label="Renombrar ${escapeHtml(content.name)}">Renombrar</button><button type="button" class="cluster-card-action" data-cluster-card-action="adjust" aria-label="Ajustar tamaño y posición de ${escapeHtml(content.name)}">Ajustar</button></div>`; card.innerHTML = `<header class="cluster-card-header"><div class="cluster-move-handle active" role="button" aria-label="Arrastrar tarjeta de cluster" title="Arrastrar tarjeta"><span class="cluster-card-drag-mark" aria-hidden="true">⠿</span><strong>${escapeHtml(content.name)}</strong><span class="cluster-count">${content.badge}</span></div></header><small class="cluster-counts">${escapeHtml(content.detail)}</small><div class="cluster-card-adaptive-content">${clusterCardMemberMarkup(members, editing)}</div>${directActions}`;
      if (editing) {
      ...
      card.querySelectorAll('[data-cluster-card-action]').forEach(action => { action.onpointerdown = event => event.stopPropagation(); action.onclick = event => { event.preventDefault(); event.stopPropagation(); if (action.dataset.clusterCardAction === 'rename') openAreaRename(area.id); else beginClusterCardEdit(area.id); }; });
      ...
      card.onclick = event => { if (card.dataset.suppressClusterOpen === 'true') { delete card.dataset.suppressClusterOpen; event.preventDefault(); event.stopPropagation(); return; } if (editing || event.target.closest('.cluster-move-handle, .cluster-card-edit-controls, .cluster-resize-handle, .cluster-member-link, .cluster-card-actions')) return; openAreaDetail(area.id); };
      card.onkeydown = event => { if (!editing && !event.target.closest('.cluster-card-actions') && (event.key === 'Enter' || event.key === ' ')) { event.preventDefault(); openAreaDetail(area.id); } };
  ...
  function refreshManagedAreaCard(areaId) { const area = managedArea(areaId); const map = currentMap(); const host = $('pins'); if (!area || !map || area.mapId !== map.id || !host) return false; [...host.querySelectorAll('.managed-area-card')].find(card => card.dataset.areaId === area.id)?.remove(); renderManagedAreaCard(host, map, area); return true; }
  function render() {
    const map = currentMap(); if (!map) return;
    ui.mapId = map.id; const definition = grid(); $('stage').style.setProperty('--grid-columns', definition.columns); $('stage').style.setProperty('--grid-rows', definition.rows); renderGridLabels(); requestViewportRender(); const resource = resourceFor(map); const plan = $('plan'); plan.alt = map.name || 'Plano'; plan.onload = () => fitInitialMap(map.id); plan.onerror = () => reportPlanResourceFailure(map, resource, `No se pudo cargar el SVG del plano «${map.name || map.id}». Recurso esperado: ${resource}.`); if (plan.getAttribute('src') !== resource) plan.src = resource; else if (plan.complete) fitInitialMap(map.id);
    ...
    individualSeats.forEach(seat => {
      ...
      pin.onmouseenter = event => preview(event, seat, presentation); pin.onmousemove = movePreview; pin.onmouseleave = hidePreview; pins.append(pin);
    });
  ...
  function hideContextMenu() { const menu = $('context-menu'); menu.classList.remove('show'); menu.removeAttribute('data-map-id'); menu.removeAttribute('data-area-id'); }
  ...
  function getDiff(open = false, bypassBusy = false) { if (!scenarioId()) return; appState.scenarioComparison.status = 'running'; if (appState.viewMode === 'scenarios') renderScenarioComparison(); if (open) $('diff-dialog').showModal(); $('diff-title').textContent = `Diff · ${scenario()?.name || scenarioId()}`; $('diff-empty').textContent = 'Cargando cambios…'; $('diff-list').replaceChildren(); send('getScenarioDiff', { scenarioId: scenarioId() }, bypassBusy); }
  ...
  $('undo').onclick = () => { appState.bulk.undoRequested = Boolean(appState.bulk.lastCommitted); ui.undoPayload = scenarioId() ? { scenarioId: scenarioId() } : {}; $('undo-preview').textContent = 'Cargando el cambio pendiente…'; $('undo-dialog').showModal(); send('getUndoPreview', ui.undoPayload); };
  ...
  $('move-seat').onclick = () => { if (!ui.seatId) return; ui.movingSeat = true; setStatus('Selecciona el destino en el plano para mover el puesto.'); setViewMode('map'); };
  ...
  document.querySelectorAll('[data-dialog]').forEach(button => button.onclick = () => $(button.dataset.dialog).showModal());
  ...
  function setAddMode(active, context = null) { if (active) setSelectionMode(false); ui.adding = active; ui.addingContext = active ? context : null; hideContextMenu(); const button = $('add-seat'); button.classList.toggle('is-active', active); $('add-seat-label').textContent = active ? 'Haz clic en el plano' : 'Añadir puesto'; if (active) setStatus(context?.targetManagedAreaId ? `Crear puesto en «${context.areaName}»: haz clic en su posición real sobre el plano.` : 'Modo añadir puesto: haz clic en el plano'); if (!active && !ui.busyAction) setStatus(zoomStatus()); }
  ...
  $('theme').onchange = event => { const theme = uiThemeFeature.apply(event.target.value); send('saveThemePreference', { theme }, true); };
  ...
  $('plan').addEventListener('click', event => {  const box = $('plan').getBoundingClientRect(); const x = Math.max(0, Math.min(1, (event.clientX - box.left) / box.width)); const y = Math.max(0, Math.min(1, (event.clientY - box.top) / box.height)); if (ui.movingSeat && ui.seatId) { const seatId = ui.seatId; ui.movingSeat = false; moveWorkspace(seatId, x, y); return; } if (ui.adding) { const context = ui.addingContext; if (context?.targetManagedAreaId) { const area = managedArea(context.targetManagedAreaId); if (!area || area.mapId !== ui.mapId) { setAddMode(false); notify('warning', 'La zona gestionada ya no existe o ya no pertenece a este plano. No se creó ningún puesto.'); return; } } send('createSeat', payloadForScenario({ mapId: ui.mapId, x, y, ...(context?.targetManagedAreaId ? { targetManagedAreaId: context.targetManagedAreaId } : {}) })); setAddMode(false); } });
  ...
  document.addEventListener('pointerdown', event => { if (!event.target.closest('#context-menu')) hideContextMenu(); if (!event.target.closest('#more-menu')) closeMoreMenu(); if (!event.target.closest('.global-search-control, #search-results')) $('search-results').classList.add('hidden'); });
  $('context-menu').addEventListener('keydown', event => { const items = [...$('context-menu').querySelectorAll('button:not(.hidden)')]; const index = items.indexOf(document.activeElement); if (event.key === 'Escape') { event.preventDefault(); hideContextMenu(); wrap.focus(); } else if ((event.key === 'ArrowDown' || event.key === 'ArrowUp') && items.length) { event.preventDefault(); items[(index + (event.key === 'ArrowDown' ? 1 : -1) + items.length) % items.length].focus(); } else if (event.key === 'Enter' && document.activeElement?.click) { event.preventDefault(); document.activeElement.click(); } });
