(() => {
  ...
  $('theme').onchange = event => { const theme = uiThemeFeature.apply(event.target.value); send('saveUserPreferences', { theme }, true); };
  ...
  window.receiveFromNative = response => {
    ...
    if (action === 'getUserPreferencesResult' || action === 'saveUserPreferencesResult') uiThemeFeature.apply(data?.theme);
