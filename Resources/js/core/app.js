(() => {
  ...
  function render() {
    ...
    individualSeats.forEach(seat => {
      ...
      const pin = document.createElement('button'); pin.className = `pin ${presentation.qualityState === 'complete' ? 'complete' : 'partial'}${presentation.interaction.dimmed ? ' dim' : ''}${ui.problemHighlightWorkspace === seat.id && ui.problemHighlightMapId === map.id ? ' problem-highlight' : ''}${appState.filters.only && !workspaceFilterFeature.matches({ ...seat, _mapId: map.id }) ? ' hidden' : ''}`;
